...
const Login = ({ setlogin, setisLogged }) => {
    ...
    function addCredential(e) {
        ...
        if (Object.keys(signupError).length === 0) {
            axios.post("http://localhost:3001/newregister", { name, email, password })
                .then((response) => {
    ...
    function handleLogin(e) {
        ...
        axios.post('http://localhost:3001/customerdetails', { existingemail: existingemail, existingpass: existingpass })
            .then(result => {
                console.log(result)
                        ...
                        {currentState === "Login" ? (<>
                            ...
                            <>
                                ...
                                <div className='login-condition' >
                                    <label className='login-conditon-input'><input type='checkbox' required />
                                    <p>By continuing, i agree to the terms of use & privacy policy.</p></label>
                                </div>
                            </>)}
                    </div>
